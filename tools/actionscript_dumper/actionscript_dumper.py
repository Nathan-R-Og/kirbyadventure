...
class Disassembler(object):
    ...
    @property
    def nes_pc(self):
        return self.to_nes_pc(self.pc)

    #function to be called anywhere
    def to_nes_pc(self, pc):
        #get actual pc without ines header
        pc = self.pc - INES_SIZE
        #return bank:ram
        return 0xA000 + (((pc >> 0xD) << 0x10) | (pc & 0x1FFF))
    ...
    def datatype_to_str(self, data_type, bytes_):
        ...
        if data_type == 'label_16':
            valid = self.symbols.get((self.nes_pc & 0xFF0000) | value, '${:04X}'.format(value))
            if type(valid) == disasm_symbol:
                valid = valid.label
            return valid
        elif data_type == 'label_24':
            valid = self.symbols.get(value, '${:06X}'.format(value))
            if type(valid) == disasm_symbol:
                valid = valid.label
            return valid
    ...
    def try_add_label(self):
        ...
        if self.force_label:
            if not label:
                ...
                self.symbols[self.nes_pc] = disasm_symbol(label)

            label = '\n' + label.label

        if label:
            if type(label) == disasm_symbol:
                label = label.label
    ...
    def disassemble_all(self):
        ...
        for start in l:
            ...
            while self.nes_pc in self.traversed:
                ...
                #check if meets the size requirement
                if self.symbols[start].size != -1:
                    self.force_label |= (self.pc - pcstart) >= self.symbols[start].size
    ...
    def disasm_opcode(self, op_byte):
        ...
        if mnemonic in ('ASMCALL', 'ASMCALL_l'):
            ...
            if asm_func:
                ...
                if "name" in list(asm_func.keys()):
                    operands[0] = asm_func['name']
    ...
    def traverse(self, start):
        ...
        while self.nes_pc not in self.traversed:
            ...
            elif (op_byte & 0xF0 == 0xD0) or op_byte == 0x26:  # ASMCALL/ASMCALL_l
                if op_byte == 0x26:
            ...
            elif op_byte in (0x29, 0x3E): # TABLECALL/MULTIJSL
                count = self.read_rom(1)
                for i in range(count):
                    addr = self.read_rom(3)
                    self.add_label(addr, f'L_{addr:06X}')
    ...
    def parse_sym_file(self):
        for i, line in enumerate(self.sym_file):
            ...
            if '=' in pre_comment:
                ...
                #check if label doesn't have proper ascii characters
                ...
                try:
                    ...
                    size = -1
                    #has no parameters
                    if str_address.find(",") == -1:
                        address = int(str_address, 16)
                    #has other parameters
                    else:
                        str_address = str_address.split(",")
                        address = int(str_address[0], 16)
                        size = int(str_address[1], 16)
                    self.add_label(address, label, size)
                except ValueError:
        ...
        for i, line in enumerate(self.asm_functions_file):
            ...
            if '=' in pre_comment:
                ...
                if key == 'COMMENT':
                    ...
                    self.asm_functions[address]['name'] = value
                else:
...
if __name__ == '__main__':
    ...
    with open(script_dir+'/symbols_out.txt', 'w') as f:
